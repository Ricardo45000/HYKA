...
GARMIN_USER_ID="fc600af9-2926-4b86-b841-25a25d17c10c"
...
if [ "$(echo "$ACTIVITY" | jq '. | length')" -gt 0 ]; then
  echo "✅ Activity found:"
  echo "$ACTIVITY" | jq '.[0]'
  
  ACTIVITY_UUID=$(echo "$ACTIVITY" | jq -r '.[0].id')
  ...
  if [ "$(echo "$FIT_FILE" | jq '. | length')" -gt 0 ]; then
    echo "✅ FIT file found:"
    echo "$FIT_FILE" | jq '.[0]'
  else
    echo "⚠️ No FIT file found yet"
  fi
  ...
  if [ "$SAMPLE_COUNT" -gt 0 ]; then
    ...
    echo "⚠️ No samples found yet (may still be processing)"
  fi
else
  echo "⚠️ Activity not found in database"
