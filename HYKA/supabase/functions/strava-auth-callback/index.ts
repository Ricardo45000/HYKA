  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      if (error) {
        ...
        const appRedirectURL = `app.hyka.com://callback?error=${encodeURIComponent(error)}&state=${encodeURIComponent(state || '')}`
        return new Response(null, {
          status: 302,
          headers: {
      ...
      if (!code) {
        ...
        const appRedirectURL = `app.hyka.com://callback?error=${encodeURIComponent("Missing authorization code")}&state=${encodeURIComponent(state || '')}`
        return new Response(null, {
          status: 302,
          headers: {
            'Location': appRedirectURL,
      ...
      const appRedirectURL = `app.hyka.com://callback?code=${encodeURIComponent(code)}&state=${encodeURIComponent(state || '')}`
